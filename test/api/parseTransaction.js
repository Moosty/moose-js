...
describe('ParseOfflineRequests', function () {
	...
	describe('#httpGETPUTorPOST', function () {
		...
		it('should tell if GET request', function () {
			var requestMethod1 = MOOSE.parseOfflineRequests('blocks/getHeight');

			(requestMethod1.requestMethod).should.be.equal('GET');
		});

		it('should tell if POST request', function () {
			var requestMethod2 = MOOSE.parseOfflineRequests('accounts/open');

			(requestMethod2.requestMethod).should.be.equal('POST');
		});

		it('should tell if PUT request', function () {
			var requestMethod3 = MOOSE.parseOfflineRequests('delegates');
	...
	describe('#checkDoubleNamedAPI', function () {
		...
		it('should route to getTransactions when /transactions API is called and no secret is added', function () {
			var requestMethodGetTx = MOOSE.parseOfflineRequests('transactions', { senderId: '123' });
	...
	describe('#checkOfflineRequestBefore', function () {
		...
		it('should route accounts/open requests correctly', function () {
			var checkRequestRouting = MOOSE.parseOfflineRequests('accounts/open', { secret: '123' });
			...
			(checkRequestRouting.checkOfflineRequestBefore().requestUrl).should.be.equal('accounts?address=12475940823804898745M');
		});

		it('should route accounts/generatePublicKey requests correctly', function () {
			var checkRequestRouting = MOOSE.parseOfflineRequests('accounts/generatePublicKey', { secret: '123' });
		...
		it('should route accounts/delegates requests correctly', function () {
			var checkRequestRouting = MOOSE.parseOfflineRequests('accounts/delegates', { secret: '123', delegates: ['+f6a1b12331281fa9b17be2b4887b8c626571dc3340c2643d9f70dfb2173cfb6c'] });
		...
		it('should route transactions requests correctly', function () {
			var checkRequestRouting = MOOSE.parseOfflineRequests('transactions', { secret: '123', recipientId: '13356260975429434553M', amount: 10000000 });
		...
		it('should route signatures requests correctly', function () {
			var checkRequestRouting = MOOSE.parseOfflineRequests('signatures', { secret: '123', secondSecret: '1234' });
		...
		it('should route delegates requests correctly', function () {
			var checkRequestRouting = MOOSE.parseOfflineRequests('delegates', { secret: '123', username: 'myname' });
		...
		it.skip('should route songs requests correctly', function () {
			var options = {
				type: 0,
				genre: 2,
			...
			var checkRequestRouting = moose.api().parseOfflineRequests('songs', options);
		...
		it('should route multisignature requests correctly', function () {
			var checkRequestRouting = MOOSE.parseOfflineRequests('multisignatures', { secret: '123', secondSeret: '123', min: 2, lifetime: 5, keysgroup: ['+123', '+234'] });
	...
	describe('#transactionOutputAfter', function () {
		...
		it('should calculate crypto for accounts/open instead of using the API', function () {
			...
			var offlineRequest = MOOSE.parseOfflineRequests('accounts/open', { secret: 'unknown' });
		...
		it('should calculate crypto for accounts/generatePublicKey instead of using the API', function () {
			...
			var offlineRequest = MOOSE.parseOfflineRequests('accounts/generatePublicKey', { secret: 'unknown' });
		...
		it('should route if everything is done already as for most API calls', function () {
			...
			var offlineRequest = MOOSE.parseOfflineRequests('delegates', { secret: 'unknown' });
		...
		it('should not allow blocked API calls - enable forging', function () {
			...
			var offlineRequest = MOOSE.parseOfflineRequests('delegates/forging/enable', { secret: 'unknown' });
			var requestAnswer = offlineRequest.transactionOutputAfter();
		...
		it('should not allow blocked API calls - disable forging', function () {
			...
			var offlineRequest = MOOSE.parseOfflineRequests('delegates/forging/disable', { secret: 'unknown' });
			var requestAnswer = offlineRequest.transactionOutputAfter();
		...
		it('should not influence already finished calls - delegates', function () {
			...
			var offlineRequest = MOOSE.parseOfflineRequests('accounts/delegates', { secret: 'unknown' });
		...
		it('should not influence already finished calls - transactions', function () {
			...
			var offlineRequest = MOOSE.parseOfflineRequests('transactions', { secret: 'unknown' });
			var requestAnswer = offlineRequest.transactionOutputAfter(request);
		...
		it('should not influence already finished calls - signatures', function () {
			...
			var offlineRequest = MOOSE.parseOfflineRequests('signatures', { secret: 'unknown' });
			var requestAnswer = offlineRequest.transactionOutputAfter(request);

			(request).should.be.eql(requestAnswer);
		});

		it('should not influence already finished calls - songs', function () {
			var request = {
				'success': 'true',
				'call': 'etc'
			};

			var offlineRequest = MOOSE.parseOfflineRequests('songs', { secret: 'unknown' });
			var requestAnswer = offlineRequest.transactionOutputAfter(request);
		...
		it('should not influence already finished calls - multisignatures', function () {
			...
			var offlineRequest = MOOSE.parseOfflineRequests('multisignatures', { secret: 'unknown' });
			var requestAnswer = offlineRequest.transactionOutputAfter(request);
		...
		it('should not influence already finished calls - multisignatures/sign', function () {
			...
			var offlineRequest = MOOSE.parseOfflineRequests('multisignatures/sign', { secret: 'unknown' });
			var requestAnswer = offlineRequest.transactionOutputAfter(request);
