...
describe('vote.js', function () {
	...
	describe('#createVote', function () {
		...
		var createVote = vote.createVote,
		    vt = null,
		    publicKey = moose.crypto.getKeys('secret').publicKey,
		...
		describe('returned vote', function () {
			...
			it('should have recipientId string equal to sender', function () {
				(vt).should.have.property('recipientId').and.be.type('string').and.equal(moose.crypto.getAddress(publicKey));
			});
			...
			it('should be signed correctly', function () {
				var result = moose.crypto.verify(vt);
				(result).should.be.ok;
			});

			it('should be second signed correctly', function () {
				var result = moose.crypto.verifySecondSignature(vt, moose.crypto.getKeys('second secret').publicKey);
				(result).should.be.ok;
			});
