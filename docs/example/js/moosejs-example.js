...
// Initiate new Moose Constructor
var MOOSE = moose.api({ testnet: true, port: 4460 });
...
$(function () {
  ...
  $('#sendMoose').on('click', function (e) {
    e.preventDefault();
    var passphrase = $('#send_secret_input').val();
    var amount = $('#send_moose_input').val();
    var recipient = $('#send_moose_recipient').val();

    amount = Math.floor(amount * 100000000);

    MOOSE.sendRequest('transactions', { secret: passphrase, amount: amount, recipientId: recipient } , function (data) {
      MOOSE.lastQuery = data;
      // console.log(MOOSE.lastQuery);
      console.log(data);
  ...
  $('#submitSignMessage').on('click', function (e) {
    ...
    var signature = moose.crypto.signAndPrintMessage(message, pass);
  ...
  $('#verifySignedMessage').on('click', function (e) {
    ...
    try {
      var message = moose.crypto.verifyMessageWithPublicKey(signature, pubKey);
    } catch (e) {
  ...
  $("#submitMessageToEncrypt").on("click", function (e) {
    ...
    var encryptedObject = moose.crypto.encryptMessageWithSecret(message, secret, recipientPublicKey);
  ...
  $("#decryptMessage").on("click", function (e) {
    ...
    try {
      var message = moose.crypto.decryptMessageWithSecret(encryptedMessage, nonce, secret, senderPublicKey);
    } catch (e) {
...
function init (passphrase) {
  (function () {
    getAccount(function (accountData) {
      var accAddress = MOOSE.getAddressFromSecret(passphrase).address;
      ...
      MOOSE.sendRequest('transactions', { senderId: accAddress, recipientId: accAddress }, function (data_tx) {
        // var str = JSON.stringify(data);
        document.getElementById('transaction_details').innerHTML = JSON.stringify(data_tx, null, 2);

        MOOSE.sendRequest('accounts/delegates', { address: accAddress }, function (data_del) {
          // var str = JSON.stringify(data);
  ...
  function getAccount (callback) {
    MOOSE.sendRequest('accounts/open', { secret: passphrase }, function (data) {
      var accAddress = MOOSE.getAddressFromSecret(passphrase);
      if (data === undefined) {
        MOOSE.sendRequest('accounts', { address : accAddress.address }, function (data_acc) {
          var returnObj = {
            open: data,
            account: data_acc
          };
